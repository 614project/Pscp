...
internal static class PscpIntrinsics
{
    ...
    public static readonly IReadOnlyDictionary<string, PscpCompletionEntry> StdinMembers =
        new Dictionary<string, PscpCompletionEntry>(StringComparer.Ordinal)
        {
            ...
            ["array"] = Completion("array", "array<T>(${1:n})", "Compile-time generic sugar that lowers to specialized typed array reads.", 15, "array<T>($1)"),
            ["list"] = Completion("list", "list<T>(${1:n})", "Compile-time generic sugar that lowers to specialized typed reads and `List<T>` materialization.", 15, "list<T>($1)"),
            ["linkedList"] = Completion("linkedList", "linkedList<T>(${1:n})", "Compile-time generic sugar that lowers to specialized typed reads and `LinkedList<T>` materialization.", 15, "linkedList<T>($1)"),
            ["tuple2"] = Completion("tuple2", "tuple2<T1, T2>()", "Compile-time generic sugar that lowers to specialized tuple reads.", 2),
            ["tuple3"] = Completion("tuple3", "tuple3<T1, T2, T3>()", "Compile-time generic sugar that lowers to specialized tuple reads.", 2),
            ["tuples2"] = Completion("tuples2", "tuples2<T1, T2>(${1:n})", "Compile-time generic sugar that lowers to specialized tuple-array reads.", 15, "tuples2<T1, T2>($1)"),
            ["tuples3"] = Completion("tuples3", "tuples3<T1, T2, T3>(${1:n})", "Compile-time generic sugar that lowers to specialized tuple-array reads.", 15, "tuples3<T1, T2, T3>($1)"),
            ["nestedArray"] = Completion("nestedArray", "nestedArray<T>(${1:n}, ${2:m})", "Compile-time generic sugar that lowers to specialized typed nested-array reads.", 15, "nestedArray<T>($1, $2)"),
    ...
    public static readonly IReadOnlyDictionary<string, PscpSignatureEntry> Signatures =
        new Dictionary<string, PscpSignatureEntry>(StringComparer.Ordinal)
        {
            ...
            ["stdin.array"] = new("stdin.array<T>(n)", new[] { "n" }, "Compile-time generic sugar that lowers to specialized typed array reads."),
            ["stdin.list"] = new("stdin.list<T>(n)", new[] { "n" }, "Compile-time generic sugar that lowers to specialized typed reads and `List<T>` materialization."),
            ["stdin.linkedList"] = new("stdin.linkedList<T>(n)", new[] { "n" }, "Compile-time generic sugar that lowers to specialized typed reads and `LinkedList<T>` materialization."),
            ["stdin.tuple2"] = new("stdin.tuple2<T1, T2>()", Array.Empty<string>(), "Compile-time generic sugar that lowers to specialized tuple reads."),
            ["stdin.tuple3"] = new("stdin.tuple3<T1, T2, T3>()", Array.Empty<string>(), "Compile-time generic sugar that lowers to specialized tuple reads."),
            ["stdin.tuples2"] = new("stdin.tuples2<T1, T2>(n)", new[] { "n" }, "Compile-time generic sugar that lowers to specialized tuple-array reads."),
            ["stdin.tuples3"] = new("stdin.tuples3<T1, T2, T3>(n)", new[] { "n" }, "Compile-time generic sugar that lowers to specialized tuple-array reads."),
            ["stdin.nestedArray"] = new("stdin.nestedArray<T>(n, m)", new[] { "n", "m" }, "Compile-time generic sugar that lowers to specialized typed nested-array reads."),
